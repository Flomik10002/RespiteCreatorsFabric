...
public abstract class GreenTeaFluid extends ModFluidsTemplate {
    ...
    public Fluid getStill() {
        return ModFluidsRegister.STILL_GREEN_TEA;
    }

    @Override
    public Fluid getFlowing() {
        return ModFluidsRegister.FLOWING_GREEN_TEA;
    }

    @Override
    public Item getBucketItem() {
        return ModFluidsRegister.GREEN_TEA_BUCKET;
    }

    @Override
    protected BlockState toBlockState(FluidState state) {
        return ModFluidsRegister.GREEN_TEA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
