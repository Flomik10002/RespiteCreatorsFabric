...
public abstract class BlackTeaFluid extends ModFluidsTemplate {
    ...
    public Fluid getStill() {
        return ModFluidsRegister.STILL_BLACK_TEA;
    }

    @Override
    public Fluid getFlowing() {
        return ModFluidsRegister.FLOWING_BLACK_TEA;
    }

    @Override
    public Item getBucketItem() {
        return ModFluidsRegister.BLACK_TEA_BUCKET;
    }

    @Override
    protected BlockState toBlockState(FluidState state) {
        return ModFluidsRegister.BLACK_TEA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
