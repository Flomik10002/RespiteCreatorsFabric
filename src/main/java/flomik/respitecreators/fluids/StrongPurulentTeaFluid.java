...
public abstract class StrongPurulentTeaFluid extends ModFluidsTemplate {
    ...
    public Fluid getStill() {
        return ModFluidsRegister.STILL_STRONG_PURULENT_TEA;
    }

    @Override
    public Fluid getFlowing() {
        return ModFluidsRegister.FLOWING_STRONG_PURULENT_TEA;
    }

    @Override
    public Item getBucketItem() {
        return ModFluidsRegister.STRONG_PURULENT_TEA_BUCKET;
    }

    @Override
    protected BlockState toBlockState(FluidState state) {
        return ModFluidsRegister.STRONG_PURULENT_TEA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
