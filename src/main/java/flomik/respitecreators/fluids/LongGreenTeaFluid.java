...
public abstract class LongGreenTeaFluid extends ModFluidsTemplate {
    ...
    public Fluid getStill() {
        return ModFluidsRegister.STILL_LONG_GREEN_TEA;
    }

    @Override
    public Fluid getFlowing() {
        return ModFluidsRegister.FLOWING_LONG_GREEN_TEA;
    }

    @Override
    public Item getBucketItem() {
        return ModFluidsRegister.LONG_GREEN_TEA_BUCKET;
    }

    @Override
    protected BlockState toBlockState(FluidState state) {
        return ModFluidsRegister.LONG_GREEN_TEA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
